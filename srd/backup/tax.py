...
class Taxes:
    def __init__(self, path_params, year):
        tools.add_params_as_attr(self, path_params + '/others/XXX.csv')
        tools.add_schedule_as_attr(self, path_params + 'schedules/schedule_fed.csv')
    ...
    def file(self, hh):
        ...
        p.net_fed_tax = max(0, p.fed_tax - p.non_refund_cred)
      
        self.calc_refundable_tax_credits(p)

    def calc_taxable_income(self, p):
        p.tot_inc = (p.inc_earn
                    + p.inc_self_earn
                    + p.inc_oas
                    + p.inc_cpp
                    + p.inc_rpp
                    ...
                    + p.inc_rrsp
                    + p.inc_uccb)
        p.net_inc = p.tot_inc - p.con_rrsp
        p.tax_inc = p.net_inc # check if different

    def calc_tax(self, p):
            ind = np.searchsorted(self.l_brackets, p.tax_inc, 'right') - 1
            p.fed_tax = self.l_constant[ind] + \
                self.l_rates[ind] * (p.tax_inc - self.l_brackets[ind])

    def calc_non_refundable_tax_credits(self, p):
        p.non_refund_cred = self.rate_non_ref_tax_cred * (self.basic_amount 
                            + get_age_cred(p) + get_pension_cred(p) 
                            + get_disabled_cred(p))

    def get_age_cred(self, p):
        amount = self.age_cred_amount if p.age >= self.min_age_cred else 0
        clawback = self.age_cred_claw_rate * max(0, p.net_inc - self.age_cred_exemption)
    ...
    def get_disabled_cred(self, p):
        amount = disability_cred_amount if p.disabled else 0
        return amount
        # this is only line 316; how to take line 318 into account?

    def calc_refundable_tax_credits(self, p):
        p.refund_cred = 0
        p.refund_cred += self.calc_abatment()
        p.refund_cred += self.uccb()
        p.refund_cred += self.cctb()
        p.refund_cred += self.ncbs()
        p.refund_cred += self.canchcare()
        p.refund_cred += self.ccb()
        p.refund_cred += self.calc_WITB(hh)
        p.refund_cred += self.witbds()

    def calc_abatment(self, p):
        if self.hhold.prov == 'qc':
            p.net_fed_tax *= (1 - self.rate_abatment_qc)
    def calc_WITB(hh):
        working_income = sum([p.inc_earn + p.inc_self_earn for p in hh.sp])
        adj_family_net_income = sum([p.net_inc for p in hh.sp])
